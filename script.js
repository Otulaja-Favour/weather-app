...
function getLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        document.getElementById('current').innerHTML = `Your location: <br>
...
async function filterCities(query) {
  document.getElementById('current').innerHTML = ''; // Clear previous document.getElementById('current')
  if (!query) return;

  try {

    // Use OpenWeatherMap's geocoding API for city document.getElementById('current')
    const response = await fetch(
    ...
    if (cities.length > 0) {
      cities.forEach(city => {
        ...
        suggestion.addEventListener('click', () => {
          ...
          document.getElementById('current').innerHTML = ''; // Clear document.getElementById('current')
    ...
    console.error("Error fetching city document.getElementById('current'):", error);
  }
}
...
document.getElementById('city').addEventListener('input', (e) => {
  ...
  if (query) {
    filterCities(query); // Show city document.getElementById('current')
